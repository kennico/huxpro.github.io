...
应用[威尔逊定理]({{ site.baseurl }}{% post_url 2018-09-19-AFINT-Chap-8~9 %}#wilsons-theorem) 
...
所以可以令 $p=1115k+223$。但是因为 $1115=223\times5$，所以 $p=223\times(5k+1)$。只有当 $k=0\text{, }p=223$ 才是一个素数；这表示满足 $p\equiv 1338\pmod {1115}$ 的素数 $p$只有一个，不存在无穷多个满足条件的素数。
