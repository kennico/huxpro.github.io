...
本文只涉及了 SEH 机制的小部分内容，许多详细的细节并未提及，比如异常回调函数的调用者 ntdll 的 `RtlDispatcherException()`（负责遍历 SEH 链）和实际上完成 unwind 操作的 `RtlUnwind()`。更多的内容还要拜读 Matt Pietrek 的[原文](https://bytepointer.com/resources/pietrek_crash_course_depths_of_win32_seh.htm)。
